...
namespace Assets.CodeAssets.Cards.BlackhandCards.Powers
{
    public class DeadMansSwitch : AbstractCard
    {
        ...
        {
            action().ApplyStatusEffect(this.Owner, new DeathCurseStatusEffect(), 20);
            action().ApplyStatusEffect(this.Owner, new FumeSpewerStatusEffect(), 2);
        }
    }
